"""
Zone-Based Stochastic Fleet Size Planning Model

This module extends the fleet size planning problem to include GEOGRAPHIC ZONES.
In large metropolitan areas, it's impractical for drivers to serve orders anywhere
in the city - they should only be matched with orders in nearby zones.

Related to Paper: "Fleet Size Planning in Crowdsourced Delivery: 
                   Balancing Service Level and Driver Utilization"

Problem Setting:
    - City divided into N geographic zones (e.g., neighborhoods)
    - Drivers located in zone i can only serve orders in zone i or adjacent zones
    - Demand arrives stochastically in each zone over multiple time periods
    - Drivers can reposition between zones (incurs travel time/cost)

Key Extensions Over Base Model:
    1. **Zone-Based Matching Constraints**: 
       Driver-order assignment x[i,j] only allowed if zones are adjacent
       
    2. **Stochastic Scenario Decomposition**: 
       Multiple demand scenarios for robust optimization
       
    3. **Driver Repositioning**: 
       Model driver movement between zones (flow conservation)

Mathematical Formulation:
    maximize  E[Σ x[t,i,j] * reward - repositioning_cost]
    
    subject to:
        Zone matching:    x[t,a,b] = 0  if zone(a) not adjacent to zone(b)
        Driver flow:      m[t+1,i] = m[t,i] + arrivals - departures + repositioning
        Demand coverage:  Σ_a x[t,a,b] ≤ demand[t,b]  for each order b
        Capacity:         Σ_b x[t,a,b] ≤ 1  for each driver a

Model Types:
    - 'UB': Upper bound (perfect foresight benchmark)
    - 'LB': Lower bound (worst-case scenarios)
    - 'heatmap': Visualization of driver distribution

This approach is useful for:
    - Large cities where geographic constraints matter
    - Planning where driver repositioning is a key decision
    - Scenarios with heterogeneous demand across regions

Dependencies:
    - gurobipy: Mathematical optimization (Gurobi solver)
...
def build_model(
    m_0: List[List[int]],
    N: int,
    n_adjacent: Dict[int, List[int]],
    n_adjacent_plus: Dict[int, List[int]],
    S: int,
    D: Dict,
    D_s: Dict,
    scenarios: int,
    T: int,
    tau_ij: List[List[int]],
    avg_d: Any,
    gamma: float,
    alpha: float,
    ...
    forecast_t: int = 5,
    MHSP_gap: bool = False,
    penalty_type: str = 'TAD'
) -> Tuple[Dict, gp.Model]:
    """
    Build the MCDRP optimization model.
    
    Constructs a multi-stage stochastic programming model for delivery routing
    with zone-based matching constraints.
    
    Args:
        m_0: Initial driver distribution [zone][driver_type] → count
        N: Number of zones in the network
        n_adjacent: Adjacent zones for each zone (reachable within 1 epoch)
        n_adjacent_plus: Extended adjacent zones (includes self)
        S: Number of stochastic scenarios
        D: Deterministic demand data D[t][i][j] → demand count
        D_s: Scenario-based demand D_s[s][t][i][j]
        scenarios: Number of scenarios for stochastic optimization
        T: Planning horizon (number of epochs)
        tau_ij: Travel time matrix tau_ij[i][j] → epochs
        avg_d: Average demand rates
        gamma: Discount factor for future rewards
        alpha: Learning rate / weight parameter
        dist_matrix: Distance matrix for zone pairs
        beta: Supply-demand imbalance parameter
        model_type: 'UB' (upper bound), 'LB', or 'heatmap'
        enter_exit: Whether to model driver entry/exit dynamics
        disturb: Whether to apply demand perturbation
        p_dist: Probability of disturbance
        forecast_t: Forecast horizon (epochs)
        MHSP_gap: Whether to compute MHSP optimality gap
        penalty_type: Penalty function type ('TAD', etc.)
        
    Returns:
        Tuple of (solution_dict, gurobi_model)
    """
    # Model parameters
    Delta = 1  # Service time duration (epochs)
    delivery_deadline = 4  # Maximum delivery time window
    bar_k = delivery_deadline - Delta  # Adjusted deadline
    max_tau = 2  # Maximum travel time
    x_tot = 0
    sol = {}
    
    # Initialize solution tracking
    sol['time_hip'], sol['time_mcdrp'] = [], []
    sol['gap_hip'], sol['gap_mcdrp'] = [], []
    
    # Calculate total demand
    Demand = np.sum(np.sum(np.sum(
        D[tt][i][j] for j in n_adjacent_plus[i]) 
        for i in range(N)) 
        for tt in range(T - 5)
    )
    
    # Extended network includes dummy node for enter/exit
